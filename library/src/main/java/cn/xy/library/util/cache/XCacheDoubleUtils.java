 ...
 */
public final class XCacheDoubleUtils implements CacheConstants {

    private static final Map<String, XCacheDoubleUtils> CACHE_MAP = new HashMap<>();

    private final XCacheMemoryUtils mCacheMemoryUtils;
    private final XCacheDiskUtils mCacheDiskUtils;

    /**
     * Return the single {@link XCacheDoubleUtils} instance.
     *
     * @return the single {@link XCacheDoubleUtils} instance
     */
    public static XCacheDoubleUtils getInstance() {
        return getInstance(XCacheMemoryUtils.getInstance(), XCacheDiskUtils.getInstance());
    }
     ...
     * @return the single {@link XCacheDoubleUtils} instance
     */
    public static XCacheDoubleUtils getInstance(@NonNull final XCacheMemoryUtils cacheMemoryUtils,
                                                @NonNull final XCacheDiskUtils cacheDiskUtils) {
        final String cacheKey = cacheDiskUtils.toString() + "_" + cacheMemoryUtils.toString();
        XCacheDoubleUtils cache = CACHE_MAP.get(cacheKey);
        if (cache == null) {
            synchronized (XCacheDoubleUtils.class) {
                cache = CACHE_MAP.get(cacheKey);
                if (cache == null) {
                    cache = new XCacheDoubleUtils(cacheMemoryUtils, cacheDiskUtils);
    ...
    private XCacheDoubleUtils(XCacheMemoryUtils cacheMemoryUtils, XCacheDiskUtils cacheUtils) {
