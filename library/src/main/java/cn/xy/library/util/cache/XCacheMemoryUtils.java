 ...
 */
public final class XCacheMemoryUtils implements CacheConstants {

    private static final int DEFAULT_MAX_COUNT = 256;

    private static final Map<String, XCacheMemoryUtils> CACHE_MAP = new HashMap<>();

    private final String                       mCacheKey;
    private final LruCache<String, CacheValue> mMemoryCache;

    /**
     * Return the single {@link XCacheMemoryUtils} instance.
     *
     * @return the single {@link XCacheMemoryUtils} instance
     */
    public static XCacheMemoryUtils getInstance() {
     ...
     * @return the single {@link XCacheMemoryUtils} instance
     */
    public static XCacheMemoryUtils getInstance(final int maxCount) {
     ...
     * @return the single {@link XCacheMemoryUtils} instance
     */
    public static XCacheMemoryUtils getInstance(final String cacheKey, final int maxCount) {
        XCacheMemoryUtils cache = CACHE_MAP.get(cacheKey);
        if (cache == null) {
            synchronized (XCacheMemoryUtils.class) {
                cache = CACHE_MAP.get(cacheKey);
                if (cache == null) {
                    cache = new XCacheMemoryUtils(cacheKey, new LruCache<String, CacheValue>(maxCount));
    ...
    private XCacheMemoryUtils(String cacheKey, LruCache<String, CacheValue> memoryCache) {
        mCacheKey = cacheKey;
