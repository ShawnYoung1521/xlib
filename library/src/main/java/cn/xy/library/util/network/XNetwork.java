package cn.xy.library.util.network;
...
import cn.xy.library.util.shell.ShellUtils;
...
import static cn.xy.library.XApp.getApp;

/**
 * <pre>
 *     author: Xy
 *     time  : 14:46 2020/12/13
...
public final class XNetwork {
    ...
    public static XApp.Task<Boolean> isAvailableAsync(@NonNull final XApp.Consumer<Boolean> consumer) {
        return doAsync(new XApp.Task<Boolean>(consumer) {
            @RequiresPermission(INTERNET)
            @Override
     ...
     */
    @RequiresPermission(INTERNET)
    public static void isAvailableByPingAsync(final XApp.Consumer<Boolean> consumer) {
     ...
     */
    @RequiresPermission(INTERNET)
    public static XApp.Task<Boolean> isAvailableByPingAsync(final String ip,
                                                            @NonNull final XApp.Consumer<Boolean> consumer) {
        return doAsync(new XApp.Task<Boolean>(consumer) {
            @RequiresPermission(INTERNET)
            @Override
            public Boolean doInBackground() {
     ...
     */
    @RequiresPermission(INTERNET)
    public static void isAvailableByDnsAsync(final XApp.Consumer<Boolean> consumer) {
     ...
     */
    @RequiresPermission(INTERNET)
    public static XApp.Task isAvailableByDnsAsync(final String domain,
                                                  @NonNull final XApp.Consumer<Boolean> consumer) {
        return doAsync(new XApp.Task<Boolean>(consumer) {
            @RequiresPermission(INTERNET)
            @Override
     ...
     */
    @RequiresPermission(allOf = {ACCESS_WIFI_STATE, INTERNET})
    public static XApp.Task<Boolean> isWifiAvailableAsync(@NonNull final XApp.Consumer<Boolean> consumer) {
        return doAsync(new XApp.Task<Boolean>(consumer) {
            @RequiresPermission(allOf = {ACCESS_WIFI_STATE, INTERNET})
            @Override
            public Boolean doInBackground() {
    ...
    public static <T> XApp.Task<T> doAsync(final XApp.Task<T> task) {
        XThread.getCachedPool().execute(task);
        return task;
    }
     ...
     * @return type of network
     * <ul>
     * <li>{@link XNetwork.NetworkType#NETWORK_ETHERNET} </li>
     * <li>{@link XNetwork.NetworkType#NETWORK_WIFI    } </li>
     * <li>{@link XNetwork.NetworkType#NETWORK_4G      } </li>
     * <li>{@link XNetwork.NetworkType#NETWORK_3G      } </li>
     * <li>{@link XNetwork.NetworkType#NETWORK_2G      } </li>
     * <li>{@link XNetwork.NetworkType#NETWORK_UNKNOWN } </li>
     * <li>{@link XNetwork.NetworkType#NETWORK_NO      } </li>
     * </ul>
     */
    @RequiresPermission(ACCESS_NETWORK_STATE)
     ...
     * @return the task
     */
    public static XApp.Task<String> getIPAddressAsync(final boolean useIPv4,
                                                      @NonNull final XApp.Consumer<String> consumer) {
        return doAsync(new XApp.Task<String>(consumer) {
            @RequiresPermission(INTERNET)
            @Override
     ...
     */
    @RequiresPermission(INTERNET)
    public static XApp.Task<String> getDomainAddressAsync(final String domain,
                                                          @NonNull final XApp.Consumer<String> consumer) {
        return doAsync(new XApp.Task<String>(consumer) {
            @RequiresPermission(INTERNET)
            @Override
    ...
    public static String getSSID() {
        WifiManager wm = (WifiManager) getApp().getApplicationContext().getSystemService(WIFI_SERVICE);
        if (wm == null) return "";
    ...
    private static final Set<XApp.Consumer<WifiScanResults>> SCAN_RESULT_CONSUMERS = new CopyOnWriteArraySet<>();
    ...
    public static void addOnWifiChangedConsumer(final XApp.Consumer<WifiScanResults> consumer) {
        if (consumer == null) return;
        XThread.runOnUiThread(new Runnable() {
            @Override
            public void run() {
        ...
        sScanWifiTimer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                XThread.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        for (XApp.Consumer<WifiScanResults> consumer : SCAN_RESULT_CONSUMERS) {
                            consumer.accept(sPreWifiScanResults);
                        }
                    }
                });
    ...
    public static void removeOnWifiChangedConsumer(final XApp.Consumer<WifiScanResults> consumer) {
        if (consumer == null) return;
        XThread.runOnUiThread(new Runnable() {
            @Override
            public void run() {
    ...
    private static boolean isSameScanResultContent(ScanResult r1, ScanResult r2) {
        return r1 != null && r2 != null && XString.equals(r1.BSSID, r2.BSSID)
                && XString.equals(r1.SSID, r2.SSID)
                && XString.equals(r1.capabilities, r2.capabilities)
                && r1.level == r2.level;
    }
    ...
    public static final class NetworkChangedReceiver extends BroadcastReceiver {
            ...
            XThread.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (preSize == 0 && mListeners.size() == 1) {
                        ...
                        getApp().registerReceiver(NetworkChangedReceiver.getInstance(), intentFilter);
                    }
                }
            });
        }
            ...
            XThread.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (preSize == 1 && mListeners.size() == 0) {
                        getApp().unregisterReceiver(NetworkChangedReceiver.getInstance());
                    }
                }
            });
        }
        ...
        public void onReceive(Context context, Intent intent) {
            if (ConnectivityManager.CONNECTIVITY_ACTION.equals(intent.getAction())) {
                // debouncing
                XThread.runOnUiThreadDelayed(new Runnable() {
                    @Override
                    @RequiresPermission(ACCESS_NETWORK_STATE)
                    public void run() {
                        NetworkType networkType = XNetwork.getNetworkType();
